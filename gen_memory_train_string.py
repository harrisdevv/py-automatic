...
def Main():
    while (True):
        ...
        choice = getch()
        print ("Choose " + choice)
        obj = load_from_file_json(projectfilepath.get_abs_path(mem_train_performance_file))
        if (choice == "1"):
            ...
            if (lap_time != ""):
                obj["records"].append({"action": str(nnumber) + " numbers", "time": lap_time});
            ...
            lap_time = start_lap_time()
            if (lap_time != ""):
                obj["records"].append({"action": str(nnumber) + " cards", "time": lap_time});
            dump_to_file(projectfilepath.get_abs_path(mem_train_performance_file), obj)
        elif (choice == "3"):
            ...
            lap_time = start_lap_time()
            if (lap_time != ""):
                obj["records"].append({"action": str(nnumber) + " words", "time": lap_time});
            dump_to_file(projectfilepath.get_abs_path(mem_train_performance_file), obj)
        elif (choice == "e"):
            break
        else:
